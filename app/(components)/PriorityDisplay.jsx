...
const PriorityDisplay = ({priority}) => {
  return (
    <div className="flex justify-start align-baseline">
        ...
        <FontAwesomeIcon icon={faFire} className={`pr-1 ${priority > 6 ? "text-red-400":"text-slate-400"}`}/>
        <FontAwesomeIcon icon={faFire} className={`pr-1 ${priority > 8 ? "text-red-400":"text-slate-400"}`}/>
    </div>
  )
}
